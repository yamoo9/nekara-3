...
export default function Dashboard({ list, redirect, redirectPath, ...restProps }) {
        ...
        <WireframeBox className={styles.grid} style={{ height: null }}>
          <WireframeBox className={styles.gridItem1} style={{ height: null }}>
            ...
            { pathname.includes('profile') ?
              ...
              <WireframeBox className={styles.child2} style={{
                display: 'flex',
                alignItems: 'flex-start',
                gap: 10,
              }}>
                <button type="button" className={styles.button} disabled={!filter} onClick={() => {
                  setSearchParams()
                }}>모든 옵션 보기</button>
                <button type="button" className={styles.button} disabled={filter} onClick={() => {
                  setSearchParams({
